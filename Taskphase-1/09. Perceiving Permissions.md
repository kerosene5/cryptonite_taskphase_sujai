...
In this challenge, we had to change the owner of the file from `root` to `hacker` using the `chown` comamnd in order to get the flag.
```
...
```
## The SUID Bit
Here we used the `chmod` command with `+s` to add a SUID Bit which will aloww users which aren't owners of that file (in this case, `/flag`) to able to get ownership of that file.
```
